...
if st.button("Backfill missing BMJ/Lancet abstracts"):
    with st.spinner("Trying to fill missing abstracts for BMJ Global Health and The Lancet Global Health..."):
        result = subprocess.run(
            [sys.executable, "scripts/backfill_missing_abstracts.py", "--limit", "50"],
            cwd=Path(__file__).resolve().parent,
            capture_output=True,
            text=True,
        )
    ...
    if result.returncode == 0:
        st.success("Abstract backfill completed.")
        st.code(result.stdout)
        st.rerun()
    else:
        st.error("Abstract backfill failed.")
        st.code(result.stdout + "\n" + result.stderr)
