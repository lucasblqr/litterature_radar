...
def _render_paper_cards(df, page_key: str) -> None:
    ...
    for i, (_, row) in enumerate(df.head(n_cards).iterrows(), start=1):
        ...
        with st.container(border=True):
            ...
            if abstract:
                with st.expander("Show full abstract", expanded=False):
                    st.write(abstract)
            else:
                st.caption("Abstract not available from the metadata source.")
...
def _render_compact_table(df, page_key: str, all_scores: bool = False) -> None:
    ...
    st.caption("A lighter table with title, abstract, score, and link.")
