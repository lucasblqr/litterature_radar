...
def _extract_crossref_item(item: dict[str, Any], journal: dict[str, Any]) -> dict[str, Any] | None:
    ...
    # For journals collected through an exact Crossref ISSN endpoint, the endpoint
    # itself identifies the journal.  Do not reject valid records merely because
    # Crossref uses a slightly different container-title spelling.
    if containers and not journal.get("issns") and not any(journal_matches(c, journal) for c in containers):
        return None
...
def fetch_crossref(journal: dict[str, Any], start: date, end: date, rows: int = 1000) -> list[dict[str, Any]]:
    """Fetch journal articles from Crossref.

    When one or more ISSNs are configured, use Crossref's exact journal endpoint
    instead of fuzzy ``query.container-title`` matching.  This is especially
    important for the economics journals, where title queries were returning
    incomplete or approximate matches.
    """
    base_params = {
    ...
    issns = [str(x).strip() for x in (journal.get("issns") or []) if str(x).strip()]
    raw_items: list[dict[str, Any]] = []

    if issns:
        # Query both print/electronic ISSNs when available because publishers do
        # not always deposit the same ISSN consistently on every Crossref record.
        for issn in issns:
            url = f"https://api.crossref.org/journals/{issn}/works"
            try:
                r = _session().get(url, params=base_params, timeout=45)
                r.raise_for_status()
                batch = r.json().get("message", {}).get("items", [])
                print(f"[Crossref exact ISSN] {journal.get('name')} / {issn}: {len(batch)} records")
                raw_items.extend(batch)
            except Exception as exc:
                print(f"[Crossref exact ISSN] {journal.get('name')} / {issn}: {exc}")
            time.sleep(0.15)
    else:
        params = dict(base_params)
        params["query.container-title"] = journal.get("name", "")
        try:
    ...
    # Deduplicate records returned by both the print and online ISSN endpoints.
    unique_items: dict[str, dict[str, Any]] = {}
    for item in raw_items:
        doi = clean_text(item.get("DOI", "")).lower()
        title = clean_text(item.get("title", [""])[0] if item.get("title") else "").lower()
        key = doi or title
    ...
    if issns:
        print(f"[Crossref exact ISSN] {journal.get('name')}: {len(papers)} unique accepted papers")
