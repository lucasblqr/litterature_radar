...
# Aggregate journal pages shown inside "Team's interest". These lists define
# which journals belong to each family. The page itself always starts from all
# papers in the family published in the last 60 days; users can then narrow the
# visible list with the on-page search / abstract / saved / journal filters.
HEALTH_JOURNALS = [
    "BMJ Global Health",
...
def render_recent_journal_group_page(
    ...
    The base list has no topic or ranking restriction: it is only journal-family
    membership plus the publication-date window. Users can then optionally use
    the standard on-page filters to narrow what they see.
    """
    ...
    # Optional user-facing filters. With the defaults selected, all papers in
    # the journal family from the last N days remain visible.
